...
public class G_4_Report_Issue_Form_Controller
{
    ...
    public void submitOA(ActionEvent actionEvent) {
        ...
        if(complaintIDlbl.getText().isEmpty() || userIDTF.getText().isEmpty() || eventnameTF.getText().isEmpty() ||
                categoryCOMBO.getValue()== null || detailsTXTAR.getText().isEmpty() || incidentDateDP.getValue()==null){
            showAlert(Alert.AlertType.ERROR, "Fill up all required field");
            return;
        }
        ...
        ArrayList<ReportConcerns> reportList =readFile("VolunteerIssuereports.bin");
        if(reportList==null){
            reportList = new ArrayList<>();
        }
        reportList.add(reportConcerns);

        FileManager.writeFile("VolunteerIssuereports.bin",reportList);
        showAlert(Alert.AlertType.CONFIRMATION,"Report Submitted Successfully");
